...
int find_network_newline(char *buf, int inbuf) {
  ...
  int i;    // current location in buf

  // Go through buf until \r\n found or inbuf reached
  for(i=0;
      !((buf[i] == '\r') && (buf[i+1] == '\n')) && (i < inbuf);
      i++);
  // If the loop exited when \r\n was found, return the location it was found
  if(buf[i] == '\r')
    return i;
  // Otherwise, \r\n was not found
  else
    ...
    if ((fd = accept(listenfd, (struct sockaddr *)&peer, &socklen)) < 0) {
      ...
      while ((nbytes = read(fd, after, room)) > 0) {
        ...
        if (where >= 0) { // OK. we have a full line
          ...
          // Calculate number of chars in the line printed
          int line_len = where + 1  //account for counting from 0
                               + 1; //account for the \n after \r
          inbuf -= line_len;
          ...
          memmove(buf, (buf + line_len), inbuf);

            
        }
        // Step 6: update room and after, in preparation for the next read
        room = sizeof(buf) - inbuf*sizeof(char);
        after = buf + inbuf;
